...
- `MAX_CONCURRENT`: 1インスタンスあたりの同時コンパイル数。既定値`1`。512 MB環境では増やさない
...
- 本番イメージはフォントDBとMVPで使うフォント解析キャッシュをビルド時に生成し、各リクエストの一時キャッシュへコピーすることで無料枠の初回メモリ負荷を抑制
- クラウド保存、DB、ログインなし
...
## Render 512 MB向けのメモリ構成

本番イメージは`texlive-full`、`texlive-latex-extra`、`texlive-fonts-recommended`をインストールしません。次のDebianパッケージを基本構成にしています。
...
`bxjsarticle`が必要とする`type1cm.sty`だけは、`texlive-latex-extra`のdebからビルド時に抽出します。日本語OTFはHarano Aji Mincho RegularとGothic Mediumだけを残します。

`cache-warmup.tex`をイメージ構築中にコンパイルし、luaotfloadのフォント名DBとHarano Aji/Latin Modernの解析済みキャッシュを`/opt/tex-cache`へ保存します。リクエストごとにこの約12 MBのシードを一時キャッシュへコピーし、終了時に作業ディレクトリごと削除します。

2026-06-22のローカル測定では、Dockerへ`--memory=512m --memory-swap=512m --cpus=1`を指定し、最小tex、日本語tex、エラーtexを順に実行した結果は次のとおりです。

| 項目 | 結果 |
|---|---:|
| cgroup `memory.peak` | 429,768,704 bytes（約410 MiB） |
| OOM / OOM kill | 0 / 0 |
| 日本語PDF | 27,576 bytes、生成成功 |
| 最小PDF | 3,677 bytes、生成成功 |
| 一時ディレクトリ残数 | 0 |
| Dockerイメージ | 318,248,102 bytes（約304 MiB） |

測定値はarm64版Docker Desktop上の値で、Renderのx86_64環境では多少変動します。512 MB環境では`MAX_CONCURRENT=1`を維持してください。
